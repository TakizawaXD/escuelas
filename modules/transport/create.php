...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($driver_name) || empty($vehicle_plate)) {
        ...
        $stmt = $db->prepare("INSERT INTO transport_routes (name, driver_name, vehicle_plate, capacity) VALUES (?, ?, ?, ?)");
        if ($stmt->execute([$name, $driver_name, $vehicle_plate, $capacity])) {
            header("Location: /modules/transport/index.php");
            exit;
        } else {
            $error = 'Error al crear la ruta de transporte.';
        }
    }
}
...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 animate-fade-in">
    <div>
        <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight">Agregar Ruta</h2>
        <p class="text-slate-500 font-medium text-sm mt-1">Dar de alta un nuevo autobús y conductor.</p>
    </div>
...
<div class="bg-white p-8 md:p-12 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100/60 max-w-3xl mx-auto relative overflow-hidden group">
    ...
    <form method="POST" class="space-y-6">
        <div>
            ...
            <input type="text" name="name" required placeholder="Ej. Ruta Norte 01, Recorrido Central..."
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="driver_name" required placeholder="Nombre completo"
            ...
            <div>
                ...
                <input type="text" name="vehicle_plate" required placeholder="Ej. ABC-1234"
        ...
        <div>
            ...
            <input type="number" name="capacity" required min="1" value="40"
                   class="w-full px-5 py-3.5 bg-slate-50 hover:bg-slate-100/50 focus:bg-white border border-slate-200 text-slate-800 rounded-xl outline-none text-sm font-bold transition-all shadow-inner focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
        </div>
